...
function App() {
  ...
  const getProcessedCoins = () => {
    let processed = coins.filter(coin => 
    ...
    if (sortConfig.key) {
        ...
        processed.sort((a, b) => {
            return defaultSortOrder.indexOf(a.symbol) - defaultSortOrder.indexOf(b.symbol);
        });
    }
    return processed;
  };
  ...
  const handleSendNotification = async () => {
    ...
    try {
        await axios.post(`${API_URL}/notification`, { title: notifTitle, message: notifMsg });
        setNotifTitle(""); setNotifMsg(""); setShowAdmin(false);
        toast.success("Bildirim yollandı!", { theme: "dark" });
  ...
  return (
    <div style={{ backgroundColor: '#13131a', minHeight: '100vh', width: '100vw', color: 'white', fontFamily: 'Segoe UI, sans-serif', padding: '20px', boxSizing: 'border-box' }}>
      ...
      <style>{`
        ...
        .time-btn { background: #1e1e2e; border: 1px solid #444; color: #aaa; padding: 5px 10px; border-radius: 6px; cursor: pointer; font-size: 0.8rem; margin-left: 5px; }
      ...
      <div style={{ width: '100%', display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'15px', padding:'0 10px' }}>
          ...
          <button onClick={() => setShowAdmin(!showAdmin)} style={{ background: 'linear-gradient(45deg, #00d2ff, #007aff)', border: 'none', padding: '10px 25px', borderRadius: '30px', fontWeight: 'bold', cursor:'pointer', color:'white', boxShadow:'0 4px 15px rgba(0, 210, 255, 0.3)' }}>
             Admin Paneli
          </button>
      </div>
      ...
      <div style={{ width: '100%', display: 'flex', gap: '15px', padding: '0 10px', marginBottom: '25px', overflowX: 'auto' }}>
          {markets.map(market => (
              <button key={market.id} className={`nav-btn ${activeTab === market.id ? 'active' : ''}`} onClick={() => setActiveTab(market.id)}>
                  {market.label}
              </button>
          ))}
      </div>
      ...
      {showAdmin && (
        <div style={{ position:'fixed', top:'0', left:'0', width:'100%', height:'100%', background:'rgba(0,0,0,0.7)', zIndex:999, display:'flex', justifyContent:'center', alignItems:'center' }}>
            <div style={{ background:'#2b2b3b', padding:'30px', borderRadius:'20px', border:'1px solid #00d2ff', width:'400px', boxShadow:'0 0 50px rgba(0, 210, 255, 0.2)' }}>
                <h2 style={{marginTop:0}}>Bildirim Gönder</h2>
                <input type="text" placeholder="Başlık" value={notifTitle} onChange={e=>setNotifTitle(e.target.value)} style={{width:'100%', padding:'12px', marginBottom:'15px', borderRadius:'8px', border:'none', background:'#1e1e2e', color:'white', boxSizing:'border-box'}} />
                ...
                <div style={{display:'flex', gap:'10px'}}>
                    <button onClick={() => setShowAdmin(false)} style={{flex:1, background:'#444', border:'none', padding:'12px', borderRadius:'8px', color:'white', cursor:'pointer'}}>İptal</button>
                    <button onClick={handleSendNotification} style={{flex:1, background:'#00d2ff', border:'none', padding:'12px', borderRadius:'8px', fontWeight:'bold', cursor:'pointer'}}>GÖNDER</button>
                </div>
            </div>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: '20px', width: '100%', height: 'calc(100vh - 180px)' }}>
          ...
          <div style={{ background: '#1e1e2e', borderRadius: '20px', overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 10px 30px rgba(0,0,0,0.3)', border:'1px solid #333' }}>
              ...
              <div style={{ padding:'15px', borderBottom:'1px solid #333', background:'#252530', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                 <h3 style={{margin:0, fontSize:'1.1rem'}}>Canlı Piyasa</h3>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                          <thead style={{ position:'sticky', top:0, background:'#1e1e2e', zIndex:10 }}>
                              <tr style={{ color: '#888', textAlign: 'left', fontSize:'0.9rem' }}>
                                  ...
                                  <th style={{ padding: '15px 20px', textAlign:'right' }} onClick={() => handleSort('price')}>
                                      Fiyat {renderSortIcon('price')}
                                  </th>
                                  
                                  <th style={{ padding: '15px 20px', textAlign:'right' }} onClick={() => handleSort('change')}>
                                      Değişim {renderSortIcon('change')}
                                  </th>
                              </tr>
                          </thead>
                          <tbody>
          ...
          <div style={{ background: '#1e1e2e', borderRadius: '20px', padding: '25px', display:'flex', flexDirection:'column', boxShadow:'0 10px 30px rgba(0,0,0,0.3)', border:'1px solid #333', position:'relative' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px' }}>
                  <div>
                    <h2 style={{ margin: 0, fontSize:'1.8rem' }}>{selectedCoin} Grafiği</h2>
                    <span style={{ color:'#888' }}>Performans Analizi</span>
                  </div>
                  <div style={{ display:'flex', alignItems:'center' }}>
                      <span style={{fontSize:'0.8rem', color:'#666', marginRight:'5px'}}>Zaman:</span>
              ...
              <div style={{ flex: 1, width: '100%', minHeight: '0' }}> 
                {chartData.length > 0 ? (
                    ...
                    <div style={{ display:'flex', height:'100%', justifyContent:'center', alignItems:'center', color:'#444', border:'2px dashed #333', borderRadius:'10px' }}>Veri yükleniyor...</div>
                )}
              </div>
          </div>
      </div>
